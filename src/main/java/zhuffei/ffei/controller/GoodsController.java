...
//        String path = System.getProperty("user.dir") + "\\src\\main\\resources\\static\\goodsImg";
  ...
  public Map changeState(@RequestBody Map map) {
    try {
      ...
      if (state == 1 || state == -1 || goodsService.getState(gid) == 2) {
  ...
  @ResponseBody
  @RequestMapping("listUncheckGoods")
  public Map listUncheckGoods(@RequestBody Map map) {

    int pageNumber = (int) map.get("pageNumber");
    int pageSize = (int) map.get("pageSize");
    Map map1 = Return.ok(goodsService.listUncheckGoods(pageNumber, pageSize));
    map1.put("total", goodsService.countUnckeckGoods());
    return map1;
  }
  ...
  @ResponseBody
  @RequestMapping("listReportedGoods")
  public Map listReportedGoods(@RequestBody Map map) {
    int pageNumber = (int) map.get("pageNumber");
    int pageSize = (int) map.get("pageSize");
    Map map1 = Return.ok(goodsService.listReportedGoods(pageNumber, pageSize));
    map1.put("total", goodsService.countReportedGoods());
    return map1;
  }
